...
dexuseu <- window(DEXUSEU['2015-11-03::2017-11-03'])
...
sma5 <- SMA(Cl(dexuseu), n = 5)
sma21 <- SMA(Cl(dexuseu), n = 21)
# Technical Analysis Chart
# barChart(dexuseu)
# addSMA(n=5,col=4)
# addSMA(n=21,col=6)
# Manual Chart
# plot(Cl(dexuseu),main="Simple Moving Averages SMA(5 & 21)")
# lines(sma5,col=4)
# lines(sma21,col=6)

# Exponential Moving Averages EMA(5 & 21)
ema5 <- EMA(Cl(dexuseu), n = 5)
ema21 <- EMA(Cl(dexuseu), n = 21)
# Technical Analysis Chart
# barChart(dexuseu)
# addEMA(n=5,col=4)
# addEMA(n=21,col=6)
# Manual Chart
# plot(Cl(dexuseu),main="Exponential Moving Averages EMA(5 & 21)")
# lines(ema5,col=4)
# lines(ema21,col=6)

# 2.1.2. Bollinger Bands BB(20,2)
bb <- BBands(cbind(Hi(dexuseu), Lo(dexuseu), Cl(dexuseu)), n = 20, sd = 2)
# Technical Analysis Chart
# barChart(dexuseu)
# addBBands(n=20,sd=2)
# Manual Chart
# plot(Cl(dexuseu),main="Bollinger Bands BB(20,2)")
...
sar <- SAR(cbind(Hi(dexuseu), Lo(dexuseu)), accel = c(0.02, 0.2))
# Technical Analysis Chart
# barChart(dexuseu)
# addSAR(accel=c(0.02, 0.2))
# Manual Chart
# plot(Cl(dexuseu),main="Parabolic Stop and Reverse SAR(0.02,0.2)")
# points(sar,col=4)
...
adx <- ADX(cbind(Hi(dexuseu), Lo(dexuseu), Cl(dexuseu)), n = 14)
# Technical Analysis Chart
# barChart(dexuseu)
# addADX(n=14)

# 2.2.2. Commodity Channel Index CCI(20,0.015)
cci <- CCI(cbind(Hi(dexuseu), Lo(dexuseu), Cl(dexuseu)), n = 20, c = 0.015)
# Technical Analysis Chart
# barChart(dexuseu)
# addCCI(n=20,c=0.015)

# 2.2.3. Moving Averages Covergence/Divergence MACD(12,26,9)
macd <- MACD(Cl(dexuseu),
             nFast = 12,
             nSlow = 26,
             nSig = 9)
# Technical Analysis Chart
...
roc <- ROC(dexuseu, n = 21)
# Technical Analysis Chart
# barChart(dexuseu)
# addROC(n=21)

# 2.2.5. Relative Strength Index RSI(14)
rsi <- RSI(Cl(dexuseu), n = 14)
# Technical Analysis Chart
# barChart(dexuseu)
# addRSI(n=14)
...
smi <-
  SMI(
    cbind(Hi(dexuseu), Lo(dexuseu), Cl(dexuseu)),
    n = 13,
    nFast = 2,
    nSlow = 25,
    nSig = 9
...
# 2.2.7. Williams %R(14)
wpr <- WPR(cbind(Hi(dexuseu), Lo(dexuseu), Cl(dexuseu)), n = 14)
# Technical Analysis Chart
# barChart(dexuseu)
# addWPR(n=14)
...
sma5tr <-
  Lag(ifelse(
    Lag(Cl(dexuseu)) < Lag(sma5) &
      Cl(dexuseu) > sma5,
    1,
    ifelse(Lag(Cl(dexuseu)) > Lag(sma5) & Cl(dexuseu) < sma5, -1, 0)
  ))
sma5tr[is.na(sma5tr)] <- 0
sma21tr <-
  Lag(ifelse(
    Lag(Cl(dexuseu)) < Lag(sma21) &
      Cl(dexuseu) > sma21,
    1,
    ifelse(Lag(Cl(dexuseu)) > Lag(sma21) & Cl(dexuseu) < sma21, -1, 0)
  ))
...
ema5tr <-
  Lag(ifelse(
    Lag(Cl(dexuseu)) < Lag(ema5) &
      Cl(dexuseu) > ema5,
    1,
    ifelse(Lag(Cl(dexuseu)) > Lag(ema5) & Cl(dexuseu) < ema5, -1, 0)
  ))
ema5tr[is.na(ema5tr)] <- 0
ema21tr <-
  Lag(ifelse(
    Lag(Cl(dexuseu)) < Lag(ema21) &
      Cl(dexuseu) > ema21,
    1,
    ifelse(Lag(Cl(dexuseu)) > Lag(ema21) & Cl(dexuseu) < ema21, -1, 0)
  ))
...
bbtr <-
  Lag(ifelse(
    Lag(Cl(dexuseu)) < Lag(bb[, 1]) &
      Cl(dexuseu) > bb[, 1],
    1,
    ifelse(Lag(Cl(dexuseu)) < Lag(bb[, 3]) & Cl(dexuseu) > bb[, 3], -1, 0)
  ))
bbtr[is.na(bbtr)] <- 0
...
sartr <-
  Lag(ifelse(
    Lag(Cl(dexuseu)) < Lag(sar) &
      Cl(dexuseu) > sar,
    1,
    ifelse(Lag(Cl(dexuseu)) > Lag(sar) & Cl(dexuseu) < sar, -1, 0)
  ))
sartr[is.na(sartr)] <- 0
...
ccismatr <-
  Lag(ifelse(
    Lag(Cl(dexuseu)) < Lag(sma5) &
      ...
      cci < (-100),
    1,
    ifelse(Lag(Cl(dexuseu)) > Lag(sma5) & Cl(dexuseu) < sma5 & cci > 100, -1, 0)
  ))
...
rocsmatr <-
  Lag(ifelse(
    Lag(Cl(dexuseu)) < Lag(sma5) &
      ...
      roc[, 4] < (-0.10),
    1,
    ifelse(Lag(Cl(dexuseu)) > Lag(sma5) & Cl(dexuseu) < sma5 &
             roc[, 4] > 0.10, -1, 0)
  ))
...
rsismatr <-
  Lag(ifelse(
    Lag(Cl(dexuseu)) < Lag(sma5) &
      ...
      rsi < 30,
    1,
    ifelse(Lag(Cl(dexuseu)) > Lag(sma5) & Cl(dexuseu) < sma5 & rsi > 70, -1, 0)
  ))
...
wprsmatr <-
  Lag(ifelse(
    Lag(Cl(dexuseu)) < Lag(sma5) &
      ...
      wpr > 0.80,
    1,
    ifelse(Lag(Cl(dexuseu)) > Lag(sma5) & Cl(dexuseu) < sma5 & wpr < 0.20, -1, 0)
  ))
