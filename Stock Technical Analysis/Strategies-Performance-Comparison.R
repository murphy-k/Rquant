...
aapl <- window(AAPL['2007-11-03::2017-11-03'])
...
ccismatr <-
  Lag(ifelse(
    Lag(Cl(aapl)) < Lag(sma5) &
      ...
      cci < (-100),
    1,
    ifelse(Lag(Cl(aapl)) > Lag(sma5) & Cl(aapl) < sma5 & cci > 100, -1, 0)
  ))
...
rsismatr <-
  Lag(ifelse(
    Lag(Cl(aapl)) < Lag(sma5) &
      ...
      rsi < 30,
    1,
    ifelse(Lag(Cl(aapl)) > Lag(sma5) & Cl(aapl) < sma5 & rsi > 70, -1, 0)
  ))
...
wprsmatr <-
  Lag(ifelse(
    Lag(Cl(aapl)) < Lag(sma5) &
      ...
      wpr > 0.80,
    1,
    ifelse(Lag(Cl(aapl)) > Lag(sma5) & Cl(aapl) < sma5 & wpr < 0.20, -1, 0)
  ))
