...
zscore <- zscore.fun(diff(log(Cl(UUP)), lag = 1), n = period)
plot.zoo(
  zscore,
  xlab = "Date",
  ylab = "Z-Score",
...
abline(h = last(zscore$UUP.Close), col = "blue")
# 4. Initialization ####
# 4.1. Strategy Name
...
bh.ret <- Return.calculate(UUP$UUP.Adjusted, method = "log")
