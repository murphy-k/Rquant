...
Calculate the log returns of "LMB". Latest observations:
...
(
  ...
  geom_density() +
  geom_rug(alpha = 0.5) + ggtitle("LMB Log-Returns %")
```
...
```{r RW Simulation, echo=FALSE}
set.seed(1)
N <- 20
mu <- mean_log_returns
sigma <- sd_log_retuns
day <- 1:N
price_init <- LMB$LMB.Adjusted[[nrow(LMB$LMB.Adjusted)]]
LMB$LMB.Adjusted[[nrow(LMB$LMB.Adjusted)]]
# Simulate prices
...
price_sim %>%
  ...
  geom_line() +
  ggtitle(str_c("LMB: Simulated Prices for ", N, " Trading Days"))
```
...
print(c("Starting Price: ", round(LMB$LMB.Adjusted[[nrow(LMB)]], digits = 3)))
...
CAGR Comparison:
```{r CAGR Calculations, echo=FALSE}
N_hist <- nrow(LMB) / N
p_start_hist <- LMB$LMB.Adjusted[[1]]
p_end_hist <- LMB$LMB.Adjusted[[nrow(LMB)]]
N_sim <- N / N
...
print(c("LMB CAGR %: ", round(CAGR_hist, digits = 4) * 100))
