...
Calculate the log returns of "BABA". Latest observations:
...
(log_returns * 100) %>%
  ...
  geom_density() +
  geom_rug(alpha = 0.5) + ggtitle("BABA Log-Returns %")
```
...
sigma <- sd_log_retuns
day <- 1:N
price_init <- BABA$BABA.Adjusted[[nrow(BABA$BABA.Adjusted)]]
BABA$BABA.Adjusted[[nrow(BABA$BABA.Adjusted)]]
# Simulate prices
set.seed(1)
...
price_sim %>%
  ...
  geom_line() +
  ggtitle(str_c("BABA: Simulated Prices for ", N, " Trading Days"))
```
...
print(c("Starting Price: ", round(BABA$BABA.Adjusted[[nrow(BABA)]],digits = 3)))
...
N_hist <- nrow(BABA) / 252
p_start_hist <- BABA$BABA.Adjusted[[1]]
p_end_hist <- BABA$BABA.Adjusted[[nrow(BABA)]]
N_sim <- N / 252
...
print(c("BABA CAGR %: ", round(CAGR_hist,4)*100))
