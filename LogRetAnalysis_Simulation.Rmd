...
Calculate the log returns of "GE". Latest observations:
```{r Calculate Log Returns, echo=TRUE}
GE %>%
  Ad() %>%
...
(
  ...
  geom_density() +
  geom_rug(alpha = 0.5) + ggtitle("GE Log-Returns %")
```
...
```{r Average Daily Percent Return, echo=TRUE}
...
```{r RW Simulation, echo=TRUE}
N <- 252
mu <- mean_log_returns
sigma <- sd_log_retuns
day <- 1:N
price_init <- GE$GE.Adjusted[[nrow(GE$GE.Adjusted)]]
GE$GE.Adjusted[[nrow(GE$GE.Adjusted)]]
# Simulate prices
set.seed(1)
...
```{r RW Visual, echo=TRUE}
# Visualize the price simulation
price_sim %>%
  ggplot(aes(day, price)) +
  geom_line() +
  ggtitle(str_c("GE: Simulated Prices for ", N, " Trading Days"))
```
...
```{r Monte Carlo Code, echo=TRUE, warning=FALSE}
...
Boundaries:
```{r End Simulation Quartiles, echo=TRUE, message=FALSE, warning=FALSE}
print(c("Starting Price: ", round(GE$GE.Adjusted[[nrow(GE)]], digits = 3)))
...
CAGR Comparison:
```{r CAGR Calculations, echo=TRUE}
N_hist <- nrow(GE) / 252
p_start_hist <- GE$GE.Adjusted[[1]]
p_end_hist <- GE$GE.Adjusted[[nrow(GE)]]
N_sim <- N / 252
