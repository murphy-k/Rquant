...
Calculate the log returns of "HSY". Latest observations:
...
(log_returns * 100) %>%
  ...
  geom_density() +
  geom_rug(alpha = 0.5) + ggtitle("HSY Log-Returns %")
```
...
sigma <- sd_log_retuns
day <- 1:N
price_init <- HSY$HSY.Adjusted[[nrow(HSY$HSY.Adjusted)]]
HSY$HSY.Adjusted[[nrow(HSY$HSY.Adjusted)]]
# Simulate prices
set.seed(1)
...
price_sim %>%
  ...
  geom_line() +
  ggtitle(str_c("HSY: Simulated Prices for ", N, " Trading Days"))
```
...
print(c("Starting Price: ", round(HSY$HSY.Adjusted[[nrow(HSY)]],digits = 3)))
...
N_hist <- nrow(HSY) / 252
p_start_hist <- HSY$HSY.Adjusted[[1]]
p_end_hist <- HSY$HSY.Adjusted[[nrow(HSY)]]
N_sim <- N / 252
...
print(c("HSY CAGR %: ", round(CAGR_hist,4)*100))
