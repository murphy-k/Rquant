...
# Load 30-min .csv ES data
# 17,520 30min bars in 1-year
df <-  read.csv("/Users/murphyk/Datasets/ES.csv", header = FALSE)
df <-
  `colnames<-`(df, c('Date', 'Time', 'Open', 'High', 'Low', 'Close', 'Volume'))
df <- as.xts(df)
# RSI Optimization ####
# [FAIL] need to index the data via datetime!
init.portf <- start(df) -86400
start.date <- start(df)
Sys.setenv(TZ = "UTC")
init.equity <- 10000
enable_stops <- FALSE
