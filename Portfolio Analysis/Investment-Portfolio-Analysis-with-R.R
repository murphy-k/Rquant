...
mcash = mtreasury[, 1]
mcashret = (mcash / 100) / 12
colnames(mcashret) = "mcashret"
acashret = mcash / 100
colnames(acashret) = "acashret"
plot(mcashret,
     main = "1 Month U.S. Treasury Bills Returns",
     xlab = "Dates",
     ylab = "Monthly Returns")
table.AnnualizedReturns(mcashret)
charts.PerformanceSummary(mcashret)

# 2.2. Fixed Income or Bonds

# 2.2.1. U.S. Total Bond Market (Barclays U.S. Aggregate Bond Market, 
# Vanguard VBMFX Mutual Fund, U.S. Investment Grade Bond Market)

getSymbols("VBMFX",
           src = "yahoo",
           from = date_start,
           to = date_end)
mbonds = to.monthly(VBMFX)
mbonds = mbonds[, 6]
mbondsret = mbonds / Lag(mbonds) - 1
mbondsret[is.na(mbondsret)] <- 0
colnames(mbondsret) <- c("mbondsret")
abondsret = mbonds / Lag(mbonds, 12) - 1
abondsret[is.na(abondsret)] <- 0
colnames(abondsret) <- c("abondsret")
plot(mbonds,
...
plot(mbondsret,
     ...
     xlab = "Dates",
     ylab = "Monthly Returns")
table.AnnualizedReturns(mbondsret)
charts.PerformanceSummary(mbondsret)

# 2.2.2. U.S. Short Term Bond Market (Barclays U.S. Short Term Treasuries Index,
# Vanguard VFISX Mutual Fund, U.S. Treasury Notes with one to three years maturity)
getSymbols("VFISX",
           src = "yahoo",
           from = date_start,
           to = date_end)
mshort = to.monthly(VFISX)
mshort = mshort[, 6]
...
# 2.2.3. U.S. Long Term Bond Market (Barclays U.S. Long Term Treasuries Index, Vanguard VUSTX Mutual Fund, U.S. Treasury Bonds with more than ten years maturity)
getSymbols("VUSTX",
           src = "yahoo",
           from = date_start,
           to = date_end)
...
usbondscomp = cbind(mcashret, mshortret, mbondsret, mlongret)
...
bondscomp = cbind(mbondsret, mibondsret)
...
assetscomp <-
  cbind(mbondsret, mstocksret, moilret, mgoldret, mreitret, mbroadret)
...
# 3.4.1. Correlation
aassetsdata <- data.frame(acashret, abondsret, astocksret)
...
# Fama-French-Carhart Factors Data
alargeq = to.yearly(VFINX)
alargeq = alargeq[, 6]
alargeretq = alargeq / Lag(alargeq) - 1
alargeretq[is.na(alargeretq)] <- 0
colnames(alargeretq) <- c("alargeretq")
amomfactors <-
  read.csv("Investment-Portfolio-Analysis-Factors-Data.txt", header = T)
amomentumq <- xts(amomfactors[, 2], order.by = as.Date(amomfactors[, 1]))
afactorsq <-
  xts(amomfactors[, 3:6], order.by = as.Date(amomfactors[, 1]))
amomentum = amomentumq[81:89, 1] / 100
aspremiumffc = afactorsq[81:89, 1] / 100
asize = afactorsq[81:89, 2] / 100
astyle = afactorsq[81:89, 3] / 100
ariskffc = afactorsq[81:89, 4] / 100

# Fama-French-Carhart Regression
time(alargeretq)[6] <- as.Date("2011-12-31")
alpremiumffc <- alargeretq - ariskffc
afactorsffc <- cbind(aspremiumffc, asize, astyle, amomentum)
aregressionffc <- lm(alpremiumffc ~ afactorsffc)

# Fama-French-Carhart Beta Coefficients
aspremiumbetaffc <- summary(aregressionffc)$coefficients[2]
asizebeta <- summary(aregressionffc)$coefficients[3]
astylebeta <- summary(aregressionffc)$coefficients[4]
amomentumbeta <- summary(aregressionffc)$coefficients[5]
abetasffc <-
  cbind(aspremiumbetaffc, asizebeta, astylebeta, amomentumbeta)
abetasffc

# Fama-French-Carhart Alpha Intercept
aalphaffc <- summary(aregressionffc)$coefficients[1]
aalphaffc

# Fama-French-Carhart Expected Returns
aretffc <-
  mean(ariskffc) + aalphaffc + aspremiumbetaffc * mean(aspremiumffc) + asizebeta * mean(asize) + astylebeta * mean(astyle) + amomentumbeta * mean(amomentum)
aretffc

# 3.5.3. Arbitrage Pricing Theory Model APT

# Arbitrage Pricing Theory Factors Data
acpiq = Quandl(
  "FRED/CPIAUCSL",
  type = "xts",
  collapse = "annual",
...
acpi = acpiq / Lag(acpiq) - 1
acpi <- acpi[2:9]
aspremiumapt <- aspremiumffc[1:8]
ariskapt <- ariskffc[1:8]
alpremiumapt <- alargeretq - ariskapt

# Arbitrage Pricing Theory Regression
afactorsapt <- cbind(aspremiumapt, acpi)
aregressionapt <- lm(alpremiumapt ~ afactorsapt)

# Arbitrage Pricing Theory Coefficients
aspremiumbetaapt <- aregressionapt$coefficients[2]
acpibeta <- aregressionapt$coefficients[3]
abetasapt <- cbind(aspremiumbetaapt, acpibeta)
abetasapt

# Arbitrage Pricing Theory Alpha Intercept
aalphaapt <- aregressionapt$coefficients[1]
aalphaapt

# Arbitrage Pricing Theory Expected Returns
aretapt <-
  mean(ariskapt) + aalphaapt + aspremiumbetaapt * mean(aspremiumapt) + acpibeta * mean(acpi)
names(aretapt) <- "aretapt"
aretapt
...
ereturns <- cbind(aalphacapm, aalphaffc, aalphaapt)
ereturns
...
mmaxlargelev = 1 / abs(Return.annualized(min(mlargeret), scale = 12) - max(acashret))
mmaxlargelev

# Maximum Annual Leverage Ratio
amaxlargelev = 1 / abs(min(alargeret) - max(acashret))
amaxlargelev
...
mnaiveret <-
  0.25 * mbondsret + 0.25 * mstocksret + 0.25 * mibondsret + 0.25 * mistocksret
colnames(mnaiveret) <- "mnaiveret"

# 4.2.2. Roche Global Portfolio
mrocheret <-
  0.24 * mbondsret + 0.18 * mstocksret + 0.33 * mibondsret + 0.25 * mistocksret
colnames(mrocheret) <- "mrocheret"

# 4.2.3. Bogle U.S. 60% Stocks, 40% Bonds Portfolio
mbogleret <- 0.40 * mbondsret + 0.60 * mstocksret
colnames(mbogleret) <- "mbogleret"

# 4.2.4. Ferri Core Four Portfolio
mferriret <-
  0.20 * mbondsret + 0.48 * mstocksret + 0.24 * mistocksret + 0.08 * mreitret
...
mportret <- cbind(mbondsret, mstocksret, mibondsret, mistocksret)
...
mmeanpret <-
  mbondsw1 * mbondsret + mstocksw1 * mstocksret + mibondsw1 * mibondsret + mistocksw1 * mistocksret
...
mstdevpret <-
  mbondsw2 * mbondsret + mstocksw2 * mstocksret + mibondsw2 * mibondsret + mistocksw2 * mistocksret
...
msharpepret <-
  mbondsw3 * mbondsret + mstocksw3 * mstocksret + mibondsw3 * mibondsret + mistocksw3 * mistocksret
...
mtailpret <-
  mbondsw4 * mbondsret + mstocksw4 * mstocksret + mibondsw4 * mibondsret + mistocksw4 * mistocksret
colnames(mtailpret) <- "mtailpret"

# 4.3.5. Optimized Portfolios Returns Comparison
optcomp = cbind(mnaiveret, mmeanpret, mstdevpret, msharpepret, mtailpret)
...
# Weak From Efficient Market Hypothesis Data
amomfactemh <-
  read.csv("Investment-Portfolio-Analysis-Factors-Data.txt", header = T)
afactemhq <-
  xts(amomfactemh[, 3:6], order.by = as.Date(amomfactemh[, 1]))
amomemhq <- xts(amomfactemh[, 2], order.by = as.Date(amomfactemh[, 1]))
atechemh = amomemhq / 100
colnames(atechemh) <- "atech"
aspremiumemh = afactemhq[, 1] / 100

# Weak Form Efficient Market Hypothesis Regression
aweakemh <- lm(aspremiumemh ~ atechemh)
summary(aweakemh)$coefficients

# Weak Form Efficient Market Hypothesis Residuals White Noise
aweakemhres <- aweakemh$residuals
aweakemhres1 <- Lag(aweakemhres)
aweakemhres1[1] <- 0
colnames(aweakemhres1) <- "aweakemhres1"
aweakreswn <- lm(aweakemhres ~ aweakemhres1)
summary(aweakreswn)$coefficients

# 5.2. Semi-Strong From Efficient Market Hypothesis

# Semi-Strong Form Efficient Market Hypothesis Data
asizeemh = afactemhq[, 2] / 100
astyleemh = afactemhq[, 3] / 100
ariskemh = afactemhq[, 4] / 100

# Semi-Strong Form Efficient Market Hypothesis Regression
afundemh <- cbind(atechemh, asizeemh, astyleemh)
asemiemh <- lm(aspremiumemh ~ afundemh)
summary(asemiemh)$coefficients

# Semi-Strong Form Efficient Market Hypothesis Residuals White Noise
asemiemhres <- asemiemh$residuals
asemiemhres1 <- Lag(asemiemhres)
asemiemhres1[1] <- 0
colnames(asemiemhres1) <- "asemiemhres1"
asemireswn <- lm(asemiemhres ~ asemiemhres1)
summary(asemireswn)$coefficients
